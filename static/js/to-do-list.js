...
function showEditModal(row) {
    ...
    let s_date = row.data()['created_at'];
...
function compareDates(s_date, e_date) {
    ...
    let created_at = new Date(arr_s[0], arr_s[1], arr_s[2]).getTime();

    let arr_e = e_date.split("-");
    let end_date = new Date(arr_e[0], arr_e[1], arr_e[2]).getTime();

    if (created_at >= end_date) {  // if start date is later than end date, there must be some problems
        return false;
    }
    else
        return true;
